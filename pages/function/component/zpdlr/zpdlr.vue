			...
			<view class="form-card">
				<view class="" v-for="(v,i) in queryData">
					...
					<view v-show="foldMoreShow">
						...
						<view class="view-flex" v-else-if="v.type=='下拉框'&&v.codeid=='SK'" style="margin-bottom:0;">
							<text class="form-left-text">{{v.colname}}</text>
				...
				<view class="fold-title" v-for="(v,i) in tableData" @tap="tolook(v)">
					...
					<view class="multiples">
						<view class="multiple-con">
							...
							<text class="right-con">{{v.成本总金额}}</text>
						</view>
						<view class="multiple-con">
							<text class="left-con">售价总金额:</text>
							<text class="right-con">{{v.售价总金额}}</text>
						</view>
					</view>
					<view class="fold-title-flex-start fold-title-con show-dots">
		...
		methods: {
			...
			condition(){
				...
				Condition(dataes).then((res) => {
					console.log("直配单 condition res",res)
					if(res.error_code==0){
			...
			search() {
				...
				for(var i in this.queryData){
					if(this.queryData[i].type=="查询下拉框"||this.queryData[i].type=="下拉框"){
						...
						if(this.queryData[i].colname=="审核状态"){
				...
				console.log("直配单 查询已上传的数据 dataes",dataes)
				GetlistF(dataes).then((res) => {
					console.log("直配单 查询已上传的数据 res",res)
					if (res.error_code == 0) {
			...
			newOrder() {
				...
				OrderNew(dataes).then((res) => {
					...
					if (res.error_code == 0) {
						uni.navigateTo({
							url: `/pages/function/component/zpdlr/lrxd?dh=${res.djbh}&orderType=add&title=未审核`
						});
					} else {
						this.$refs.uToast.show({
							type: "error",
			...
			tolook(event){
				...
				uni.navigateTo({
					url: `/pages/function/component/zpdlr/lrxd?dh=${event.单据票号}&orderType=${tempType}&title=${event.单据状态}&fdbh=${event.分店编号}&fdmc=${event.分店名称}&sjbh=${event.商家编号}&sjmc=${event.商家名称}&zprq=${event.直配日期}&pzs=${event.品种数}&zpzl=${event.直配总量}&cbzje=${event.成本总金额}&sjzje=${event.售价总金额}`
				});
			},
