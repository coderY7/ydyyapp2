			...
			<u-form class="form-card" labelPosition="left" :model="editForm" :rules="editRules" ref="uForm">
				...
				<view>
					...
					<view class="shopTishi">
						...
						<view class="shopTishi-view-half" v-if="editForm.hsjj">
							<text class="left-con">会员价格:</text>
							<text>{{editForm.hsjj}}</text>
						</view>
					</view>
				</view>
				<u-form-item label="直配数量" :labelWidth="74" prop="zpsl">
					<u-input placeholder="请输入直配数量" type="number" v-model="editForm.zpsl">
					</u-input>
				</u-form-item>
				<u-form-item label="直配进价" :labelWidth="74" prop="zpjj">
					<u-input placeholder="请输入直配进价" type="number" v-model="editForm.zpjj">
					</u-input>
				</u-form-item>
				<u-form-item label="商品备注" :labelWidth="74" prop="remark">
					<u-input placeholder="请输入商品备注" type="text" v-model="editForm.remark">
					</u-input>
				</u-form-item>
			</u-form>
			<view class="form-card">
				...
				<view>
					<view class="radio-view">
						<view class="radio-text" v-for="(v, i) in lxlist" :class="{lxactive:editForm.cxtype==v.sjcxlxid}" @tap="formMoreChange(v.sjcxlxid)">{{v.lxmc}}</view>
					</view>
				</view>
			</view>
		...
		<view class="foldGroup" v-show="!stateDetail">
			<view class="fold-title" v-if="tobj.orderType!='add'">
				...
				<view class="fold-title-flex-start fold-title-con show-dots">
					...
					<text class="right-con">{{tobj.sjbh}}-{{tobj.sjmc}}</text>
				</view>
				<view class="multiples">
					<view class="multiple-con">
						<text class="left-con">分店编号:</text>
						<text class="right-con">{{tobj.fdbh}}</text>
					</view>
					<view class="multiple-con">
						<text class="left-con">分店名称:</text>
						<text class="right-con">{{tobj.fdmc}}</text>
					</view>
				</view>
				<view class="multiples">
					<view class="multiple-con">
						<text class="left-con">品种数:</text>
						<text class="right-con">{{tobj.pzs}}</text>
					</view>
					<view class="multiple-con">
						<text class="left-con">直配总量:</text>
						<text class="right-con">{{tobj.zpzl}}</text>
					</view>
				</view>
				<view class="multiples">
					<view class="multiple-con">
						<text class="left-con">成本总金额:</text>
						<text class="right-con">{{tobj.cbzje}}</text>
					</view>
					<view class="multiple-con">
						<text class="left-con">售价总金额:</text>
						<text class="right-con">{{tobj.sjzje}}</text>
					</view>
				</view>
				<view class="fold-title-flex-start fold-title-con show-dots">
					<text class="left-con">直配日期:</text>
					<text class="right-con">{{tobj.zprq}}</text>
				</view>
				<view class="multiples">
					<view class="multiple-con">
						<text class="left-con">单据状态:</text>
						<text class="right-con" v-if="tobj.title=='已审核'" style="color:green;">{{tobj.title}}</text>
						<text class="right-con" v-else style="color:red;">{{tobj.title}}</text>
					</view>
				</view>
			</view>
			<view class="fold-content" v-for="(item,index) in tableData">
				...
				<view class="multiples">
					...
					<view class="multiple-con view-flex">
						<text class="left-con">会员价格:</text>
						<text class="right-con">￥{{item.hsjj}}</text>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			toeditDetail(row, index) {
				...
				this.editForm.zpsl=row.phsl
				this.editForm.zpjj=row.phjg
				this.editForm.nsjg=row.nsjg
				this.editForm.hsjj=row.hsjj
			...
			editDetailSave() {
				this.$refs.uForm.validate().then(resf => {
					...
					this.uploadarr.push({
						...
						"dbsl": this.editForm.zpsl,
						"dbjg": this.editForm.zpjj,
						"hsjj": this.editForm.hsjj,
						"cxjbz": this.editForm.cxtype,
						"nsjg": this.editForm.nsjg,
						"bzqts": this.tableData[this.tableIndex].bzqts,
						"spremark": this.editForm.remark
					})
