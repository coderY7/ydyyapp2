	...
	<view class="box-compents">
		...
		<view v-show="stateDetail">
			...
			<u-form class="form-card" labelPosition="left" :model="editForm" :rules="editRules" ref="uForm">
				...
				<u-form-item label="盘点数量" :labelWidth="74" prop="spsl">
					<u-input placeholder="请输入盘点数量" type="number" v-model="editForm.spsl">
					</u-input>
				</u-form-item>
			</u-form>
		...
		<view class="foldGroup" v-show="!stateDetail">
			<view class="fold-title" v-if="title.state!='add'">
				...
				<view class="fold-title-flex-start fold-title-con show-dots">
					...
					<text class="right-con">{{title.gzdh}}</text>
				</view>
				<view class="multiples">
					...
					<view class="multiple-con">
						...
						<text class="right-con">{{title.ckbmid}}</text>
					</view>
				</view>
				<view class="multiples">
					<view class="multiple-con">
						<text class="left-con">盘点总数:</text>
						<text class="right-con">{{title.pdzs}}</text>
					</view>
					<view class="multiple-con">
						<text class="left-con">损溢总数:</text>
						<text class="right-con">{{title.syzs}}</text>
					</view>
				</view>
				<view class="multiples">
					<view class="multiple-con">
						<text class="left-con">成本总额:</text>
						<text class="right-con">{{title.pdcbze}}</text>
					</view>
					<view class="multiple-con">
						<text class="left-con">零售总额:</text>
						<text class="right-con">{{title.pdnsze}}</text>
					</view>
				</view>
				<view class="multiples">
					<view class="multiple-con">
						<text class="left-con">损溢总额:</text>
						<text class="right-con">{{title.sycbze}}</text>
					</view>
					<view class="multiple-con">
			...
			<view class="fold-content" v-for="(item,index) in tableData">
				...
				<view class="multiples">
					<view class="multiple-con view-flex">
						...
						<text class="right-con">{{item.spsl}}</text>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			console.log("EDIT this.tableData",this.tableData)
		},
		methods: {
			// 编辑商品
					...
					success: (resm)=> {
						if (resm.confirm) {
							let dataes={
								...
								"djbh": this.title.盘点单号,
								"fdbh": uni.getStorageSync("fdbh"),
								"pdmd": this.tableData[index].fdbh,
								"list": [{
									"guid": row.recordid,
									"spbm": row.spbm,
									"spmc": row.spmc,
							...
							PddDelLine(dataes).then((res) => {
								console.log("盘点删除 res",res)
								if (res.error_code == 0) {
									this.$parent.getList()
