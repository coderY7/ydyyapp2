    ...
    <view class="box-content">
      ...
      <view class="foldGroup">
        <view class="fold-title" v-for="(v,i) in tableData" @tap="tolook(v)">
          ...
          <view class="multiples">
            <view class="multiple-con">
              <text class="left-con">冲账总量:</text>
              <text class="right-con">{{v.冲账总量}}</text>
            </view>
            <view class="multiple-con">
              <text class="left-con">冲账总额:</text>
              <text class="right-con">{{v.冲账总额}}</text>
            </view>
          </view>
          <view class="multiples">
            <view class="multiple-con">
  ...
  methods: {
    ...
    newOrder(){
      ...
      OrderNew(dataes).then((res) => {
        ...
        if(res.error_code==0){
          uni.navigateTo({
            url: `/pages/function/component/pfczd/xsczxd?djbh=${res.djbh}&state=add`
          });
        }else{
          this.$refs.uToast.show({
            type:"error",
